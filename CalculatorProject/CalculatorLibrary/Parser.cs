        ...
        public List<Token> Postfix(string expression)
        {
            ...
            foreach (Token token in listOfTokens)
            {
                ...
                else
                {
                    ...
                    while (tokenStack.Count > 0 && Convert.ToString(tokenStack.Peek().Value)!="("  && operatorPrecedence[Convert.ToString(token.Value)].precedence <= operatorPrecedence[Convert.ToString(tokenStack.Peek().Value)].precedence)
                    {
                        postfixExpression.Add(tokenStack.Pop());
                    }
